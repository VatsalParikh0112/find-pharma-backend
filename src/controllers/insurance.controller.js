...
const createInsurance = async (req, res) => {
  ...
  const { providerName, planName, planType, memberId, groupNumber, holderName, rxBIN, rxPCN, rxGroup, isDefault } = req.body;

  try {
    const count = await Insurance.countDocuments({ patient: req.user._id });
    if (count >= 3) {
      return res.status(400).json({ success: false, message: 'You can save a maximum of 3 insurance cards.' });
    }
...
const getMyInsurances = async (req, res) => {
  try {
    const insurances = await Insurance.find({ patient: req.user._id }).sort({ isDefault: -1, createdAt: -1 });
...
const updateInsurance = async (req, res) => {
  ...
  try {
    ...
    if (isDefault) {
      await Insurance.updateMany({ patient: req.user._id, _id: { $ne: req.params.id } }, { isDefault: false });
    }
...
const deleteInsurance = async (req, res) => {
  try {
    const insurance = await Insurance.findOneAndDelete({ _id: req.params.id, patient: req.user._id });
    if (!insurance)
      return res.status(404).json({ success: false, message: 'Insurance not found' });
