...
const { createInsurance, getMyInsurances, updateInsurance, deleteInsurance, setDefault } = require('../controllers/insurance.controller');
...
const insuranceValidators = [
  ...
  body('planType').optional({ checkFalsy: true }).isIn(['PPO','HMO','EPO','HDHP','POS','Medicare','Medicaid','Other']).withMessage('Invalid plan type'),
  body('rxBIN').optional({ checkFalsy: true }).matches(/^\d{6}$/).withMessage('RxBIN must be exactly 6 digits'),
];
